...
TIMER implements a register (**msip**) to generate software interrupts. With reference to its configuration details tabled in Section [msip](#msip), only the least significant bit (LSB) is effective for this purpose, which
...
TIMER implement a register **msftrst** to generate a software-reset request. With reference to its configuration details provided later in Section [msftrst](#msftrst), only the most significant bit (MSB) is effective for this purpose, which
