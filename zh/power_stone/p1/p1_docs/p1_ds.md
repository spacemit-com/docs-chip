...
点击下载 [P1 数据手册 (PDF)](https://cdn-resource.spacemit.com/file/%E8%8A%AF%E7%89%87/P1/P1_Datasheet_ZH_V2.0_20250331.pdf)
...
4. **功能关闭**：
   ...
   - 可通过置位**表 7-127** SYS_CFG2[6]禁用阈值调整
...
1. 扫描通道 0 时，12-bit 结果存放在 **表 7-42** ADC_MAN_RES_H 和 **表 7-43** ADC_MAN_RES_L。
...
1. 配置完对应通道使能后，每完成一次转换，ADC 内部通道会自动切换到通道 0 下一次使能的通道，完成一轮扫描后，再切换回最初使能的通道，如下图 **ADC 通道 0 扫描示意图** 所示。
2. 如需切换扫描顺序，可在下一次转换启动前配置对应通道使能。
3. 如需从头开始扫描，有两种方法：
   - 关闭并重新使能 ADC_EN；
   - 切换至自动模式再切回手动模式：先将 ADC_AUTO 任意寄存器配置为 1，然后全清零。
4. **表 7-39** ADC_MAN_EN0 ~ **表 7-41** ADC_MAN_EN2 的通道全部扫描并转换完成后，会产生事件 **表 7-108** EVENT1[2]。如使能中断（**表 7-115** IRQ_EN1[2]），会触发中断（拉低 INT 引脚），直至软件清除事件或清零中断使能位。
