...
### 12.1.1 Video Processing Unit (VPU)
...
- Max frame width: 4096 pixels (2,048 pixels for interlaced stream)
- Max frame height: 4096 pixels
- Error concealment is performed for managing bitstream errors
...
### 12.2.1 Graphics Processing Unit (GPU)
...
| Variable                     | Bit           | Comment                |
|------------------------------|---------------|------------------------|
| C0(in), C1(in), C2(in), C3(in)       | 8bit unsigned | Input channel          |
| C0(inter), C1(inter), C2(inter)    | 10bit signed  | Intermediate channel value |
| C0(out), C1(out), C2(out), C3(out)   | 8bit unsigned | Output channel         |

| Register             | Index | Bit           | Comment               |
|----------------------|-------|---------------|-----------------------|
| LayerX_CSC_enable    | -     | 1bit unsigned | 0: disable<br>1: enable |
| Layer_matrix[#][#]   | 0-11  | 13bit signed  | [-4096, 4095]         |
...
| Variable                              | Bit                    | Comment         |
|---------------------------------------|------------------------|-----------------|
| Rect_left<br>Rect_top                 | 16bit unsigned         | [0, 65535]      |
| Rect_width<br>Rect_height             | 5bit unsigned          | [1, 16]         |
| pixel_index                           | 8bit unsigned          | [0, 65535]      |
| s0, s1, s2, s3                        | 8bit unsigned          | [0, 255]        |
| Y00, Y01, Y10, Y11, U00, U01,<br>U10, U11, V00, V01, V10, V11,<br>U, V, R, G, B, A | 8bit unsigned | [0, 255] |
| data_in[4][256]                       | 8bit unsigned × 4 × 256| [0, 255]        |

| Register           | Bit             | Comment                                                                 |
|--------------------|-----------------|-------------------------------------------------------------------------|
| Output_format      | 3bit unsigned   | 0: RGB888 (R low, B high)<br>1: RGBX8888<br>2: RGBA8888<br>3: ARGB8888 (A low, B high)<br>5: yuv420sp (U low, V high) |
| Output_swap        | 1bit unsigned   | 0: No swap<br>1: RGB swap RB, YUV swap UV                               |
| Output_layout      | 1bit unsigned   | 0: Linear<br>1: FBC compressed                                          |
| Output_crop_left   | 16bit unsigned  | Range [0, 65534]; <br>crop_left < output_left + output_width                |
| Output_crop_top    | 16bit unsigned  | Range [0, 65534]; <br>crop_top < output_top + output_height                 |
