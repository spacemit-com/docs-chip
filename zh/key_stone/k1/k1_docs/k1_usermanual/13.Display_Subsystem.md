...
#### DSI视频端口控制寄存器1（DSI Video Port Control Register 1, DSI_VPN_CTRL_1）  
偏移地址（Offset）：0x104  

位域 | 字段 | 类型 | 复位值 | 说明  
|---|----|----|----|---|  
31 | CFG_VPN_VSYNC_RST_EN | RW | 0x0 | LCD Vsync复位使能（从模式下）<br>`1`: 当LCD Vsync到来时，复位DSI垂直状态机。此功能仅在LCD处于从模式时有效。<br>`0`: 不复位DSI垂直状态机  
30:28 | RSVD | RO | 0 | 保留供将来使用  
27 | CFG_VPN_AUTO_WC_DIS | RW | 0x0 | VPN自动字计数禁用<br>此位优先级低于CFG_VPN_HACT_WC_EN<br>`0`: 启用自动字计数计算，硬件自动计算每行H线槽中发送的字节数<br>`1`: 自动字计数计算无效  
26 | CFG_VPN_HACT_WC_EN | RW | 0x0 | VPN Hact字计数使能<br>此位优先级高于CFG_VPN_AUTO_WC_EN<br>`0`: 如果CFG_VPN_AUTO_WC_DIS设置为0，则CFG_HACT_WC无效<br>`1`: 启用Hact字计数参数，CFG_HACT_WC将用于决定发送的字节数  
25 | CFG_VPN_TIMING_CHECK_DIS | RW | 0x0 | VPN Hss/Hse/Hact TX定时检查禁用<br>`0`: 在请求DPHY进行TX前检查定时<br>`1`: 请求DPHY进行TX前不检查定时  
24 | CFG_VPN_AUTO_DLY_DIS | RW | 0x0 | VPN自动Vsync延迟计数禁用<br>`0`: 启用自动Vsync延迟计数计算，硬件将自动使用CFG_HTOTAL_CNT的一半替换CFG_VPN_DLY_CNT进行Vsync延迟<br>`1`: 禁用自动Vsync延迟计数，硬件将使用CFG_VPN_DLY_CNT中的值进行Vsync延迟  
23 | RSVD | RO | 0 | 保留供将来使用  
22 | CFG_VPN_HLP_PKT_EN | RW | 0x0 | 长消隐包使能<br>`1`: DSI在HLP时间槽内发送长消隐包<br>`0`: 长消隐包禁用。DSI在此时间槽进入低功耗（大多数情况下，此字段应编程为0x0）  
21 | CFG_VPN_HEX_PKT_EN | RW | 0x0 | 额外长消隐包使能<br>`1`: DSI在像素数据传输后、hfp之前发送长消隐包<br>`0`: 额外长消隐包禁用。DSI在此时间槽进入低功耗（大多数情况下，此字段应编程为0x0）  
20 | CFG_VPN_HFP_PKT_EN | RW | 0x0 | 前沿消隐包使能<br>`1`: DSI在hfp时间槽内发送长消隐包<br>`0`: hfp长消隐包禁用。DSI在此时间槽进入低功耗。如果前沿周期不足以让DPHY进入低功耗状态并及时返回HS以准备下一个Hss包，此字段应编程为0x1  
19 | RSVD | RO | 0 | 保留供将来使用  
18 | CFG_VPN_HBP_PKT_EN | RW | 0x0 | 后沿消隐包使能<br>`1`: DSI在hbp时间槽内发送长消隐包<br>`0`: hbp长消隐包禁用。DSI在此时间槽进入低功耗。如果后沿周期不足以让DPHY进入低功耗状态并及时返回HS模式以准备下一个像素数据包，此字段应编程为0x1  
17 | CFG_VPN_HSE_PKT_EN | RW | 0x0 | Hse包使能<br>`1`: DSI在hbp时间槽内发送hse包<br>`0`: hse包禁用。DSI在此时间槽进入低功耗。<strong>注意：</strong>当传输模式为带同步脉冲的非突发模式时启用此位  
16 | CFG_VPN_HSA_PKT_EN | RW | 0x0 | Hsa包使能<br>`1`: DSI在hbp时间槽内发送hsa长消隐包<br>`0`: hsa包禁用。DSI在此时间槽进入低功耗。<br>- 如果传输模式为非突发模式（带同步事件）或突发模式，此字段应禁用。<br>- 如果传输模式为带同步脉冲的非突发模式，此字段可编程为0x1  
15 | RSVD | RO | 0 | 保留供将来使用  
14 | CFG_VPN_HEX_SLOT_EN | RW | 0x0 | 像素数据后的额外长包使能<br>`1`: 在像素数据传输后启用额外长包，这将在hfp前插入一个长消隐包<br>`0`: 在像素数据传输后不插入额外长包<br>此字段仅在突发模式下生效。在大多数情况下，此字段应编程为0x0  
13:11 | RSVD | RO | 0 | 保留供将来使用  
10 | CFG_VPN_LAST_LINE_TURN | RW | 0x0 | 在最后一行H线上转换总线<br>`1`: DSI将在每帧的最后一行水平线上转换总线。这将请求从设备返回确认或带有错误的确认。<br>`0`: DSI不会在帧的最后一行水平线上转换总线。在大多数情况下，此字段应设为0x0  
9 | CFG_VPN_LPM_FRAME_EN | RW | 0x0 | 每帧进入低功耗模式<br>`1`: DSI将在每帧的最后一行水平线上进入低功耗模式<br>`0`: DSI不会在最后一行水平线上进入低功耗模式。在大多数情况下，此字段应编程为0x0  
8:4 | RSVD | RO | 0 | 保留供将来使用  
3:2 | CFG_VPN_BURST_MODE | RW | 0x0 | DSI传输模式（针对LCD 1）<br>`0x0`: 非突发模式（带同步脉冲）<br>`0x1`: 非突发模式（带同步事件）<br>`0x2`: 突发模式  
1:0 | CFG_VPN_RGB_TYPE | RW | 0x0 | LCD 1输入数据RGB模式（针对LCD 1）<br>`0x0`: 565 RGB模式<br>`0x1`: 666打包模式<br>`0x2`: 666非打包模式<br>`0x3`: 888 RGB模式  

#### DSI视频端口时序寄存器0（DSI Video Port Timing Register 0, DSI_VPN_TIMING_0）
偏移地址（Offset）：0x110

位域 | 字段 | 类型 | 复位值 | 说明  
|---|----|----|----|---|  
31:16 | CFG_VPN_HACT_CNT | RW | 0x0 | VPN hact时钟计数（以字节时钟为单位）<br>此参数定义水平线像素数据周期的字节时钟周期数。<br>此期间的数据字节数为<br>HACT_BYTE_CNT = HACT_CNT * lane_num  
15:0 | CFG_VPN_HTOTAL_CNT | RW | 0x0 | VPN htotal时钟计数（以字节时钟为单位）<br>此参数定义水平线周期的字节时钟周期数。<br>此期间的数据字节数为<br>HTOTAL_BYTE_CNT = HTOTAL_CNT * lane_num  

#### DSI视频端口时序寄存器1（DSI Video Port Timing Register 1, DSI_VPN_TIMING_1）
偏移地址（Offset）：0x114

位域 | 字段 | 类型 | 复位值 | 说明  
|---|----|----|----|---|  
31:16 | CFG_VPN_HSYNC_CNT | RW | 0x0 | VPN hsync时钟计数（以字节时钟为单位）<br>此参数定义水平线hsync周期的字节时钟周期数。<br>此期间的数据字节数为<br>HSYNC_BYTE_CNT = HSYNC_CNT × lane_num  
15:0 | CFG_VPN_HBP_CNT | RW | 0x0 | VPN hbp时钟计数（以字节时钟为单位）<br>此参数定义水平线后沿周期的字节时钟周期数。<br>- 此期间的数据字节数为<br>HBP_BYTE_CNT = HBP_CNT × lane_num<br>- 前沿时钟计数可以通过以下公式计算：<br>HFP_CNT = HTOTAL_CNT - HSYNC_CNT - HACT_CNT - HBP_CNT<br>- 前沿周期的数据字节数为<br>HFP_BYTE_CNT = HFP_CNT × lane_num  

#### DSI视频端口时序寄存器2（DSI Video Port Timing Register 2, DSI_VPN_TIMING_2）
偏移地址（Offset）：0x118

位域 | 字段 | 类型 | 复位值 | 说明  
|---|----|----|----|---|  
31:16 | CFG_VPN_VACT_CNT | RW | 0x0 | VPN vact行计数<br>此参数定义垂直活动显示区域的行数。  
15:0 | CFG_VPN_VTOTAL_CNT | RW | 0x0 | VPN vtotal行计数<br>此参数定义整个垂直显示周期的总行数。  
